...
generate.data.norm <- function(number.of.seeds, sample.size, r1, r2){
  #create id variable
  id <- 1:sample.size
...
sim <- function(n, iter, number.of.seeds, sample.size, r1,r2, label){
  ...
  for(i in 1:n){
    #generate tree data
    net <- generate.data.norm(number.of.seeds,sample.size, r1,r2)
    #run test
...
#simulations to run
case1 <- sim(n,iter,  5, 500, 0, 0, "r1=0, r2=0")
case2 <- sim(n, iter,5, 500, 0, .25,"r1=0, r2=.25")
case3 <- sim(n,iter, 5, 500, 0, .5, "r1=0, r2=.5")
case4 <- sim(n,iter, 5, 500, 0, .75, "r1=0, r2=.75")
case5 <- sim(n,iter, 5, 500, .25, 0, "r1=.25, r2=.0")
case6 <- sim(n,iter, 5, 500, .25, .25, "r1=.25, r2=.25")
case7 <- sim(n, iter,5, 500, .25, .5, "r1=.25, r2=.5")
case8 <- sim(n, iter,5, 500, .25, .75, "r1=.25, r2=.75")
case9 <- sim(n, iter,5, 500, .5, 0, "r1=.5, r2=0")
case10 <- sim(n,iter, 5, 500, .5, .25, "r1=.5, r2=.25")
case11 <- sim(n,iter, 5, 500, .5, .5, "r1=.5, r2=.5")
case12 <- sim(n,iter, 5, 500, .5, .75, "r1=.5, r2=.75")
case13 <- sim(n,iter, 5, 500, .75, 0, "r1=.75, r2=0")
case14 <- sim(n,iter, 5, 500, .75, .25, "r1=.75, r2=.25")
case15 <- sim(n, iter,5, 500, .75, .5, "r1=.75, r2=.5")
case16 <- sim(n,iter, 5, 500, .75, .75, "r1=.75, r2=.75")

result <- rbind(case1, case2, case3, case4, case5, case6, case7, case8, case9, case10, 
                case11, case12, case13, case14, case15, case16)




write.csv(result, paste0("type1_pearson_",l,".csv"), row.names=F)
